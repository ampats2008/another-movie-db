...
const ScoreMeter: React.FC<Props> = ({
  vote_average,
  viewPortSize = "h-[50px] w-[50px]",
  viewBox = "0 0 40 40",
  ...
  return (
    <>
      <svg
        id="popularityMeter"
        className={`${viewPortSize} ${pos} p-1`}
        viewBox={viewBox}
      >
        <circle
          className={`popularityMeter ${meterColor}`}
          r="15"
          cx="50%"
          cy="49%"
          fill="rgba(0,0,0,0.4)"
          strokeWidth="4"
          strokeDasharray="94"
          strokeDashoffset={endingStrokeDashOffset}
        />
        <text
